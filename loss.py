...
class CAMLoss_km(nn.Module):
    def __init__(self, size_average=True):
        super(CAMLoss_km,self).__init__()
    ...
    def forward(self, input, label):
        ...
        label = label.view(-1)
        # loss =  (1 - label) * input_1.mean(dim=1)
        # loss += - label * input_1.mean(dim=1)

        loss = (1 - label) *self.loss(input_1,torch.zeros_like(input_1))+label*self.loss(input_1,torch.ones_like(input_1))
